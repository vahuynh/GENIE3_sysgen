function [VIM_expr,VIM_gen,VIM_sum,VIM_prod] = GENIE3_SG_sep(expr_matrix,gen_matrix,input_idx,tree_method,K,ntrees)
%Computation of tree-based weights for all putative edges using the
%GENIE3-SG-sep method.
%
%[VIM_expr,VIM_gen,VIM_sum,VIM_prod] = 
%GENIE3_SG_sep(expr_matrix,gen_matrix) learns 2*p tree models from
...
%
%[VIM_expr,VIM_gen,VIM_sum,VIM_prod] = 
%GENIE3_SG_sep(expr_matrix,gen_matrix,input_idx) only uses as candidate
...
%
%[VIM_expr,VIM_gen,VIM_sum,VIM_prod] = 
%GENIE3_SG_sep(expr_matrix,gen_matrix,input_idx,tree_method) specifies
%which tree precedure is used. Available methods are:
%   * 'RF' - Random Forests (Default method)
%   * 'ET' - Extra Trees
%
%[VIM_expr,VIM_gen,VIM_sum,VIM_prod] = 
%GENIE3_SG_sep(expr_matrix,gen_matrix,input_idx,tree_method,K)
...
%
%[VIM_expr,VIM_gen,VIM_sum,VIM_prod] =
%GENIE3_SG_sep(expr_matrix,gen_matrix,input_idx,tree_method,K,ntrees)
...
if nargin > 2 && sum(ismember(input_idx,1:nb_genes)) ~= length(input_idx)
...
%% Learn an ensemble of trees using the expression data

fprintf('Learning tree models from expression data...\n')

if nargin == 2
    VIM_expr = GENIE3(expr_matrix);
elseif nargin == 3
    VIM_expr = GENIE3(expr_matrix,input_idx);
elseif nargin == 4
    VIM_expr = GENIE3(expr_matrix,input_idx,tree_method);
elseif nargin == 5
    VIM_expr = GENIE3(expr_matrix,input_idx,tree_method,K);
else
    VIM_expr = GENIE3(expr_matrix,input_idx,tree_method,K,ntrees);
end


%% Learn an ensemble of trees using the genotype data

fprintf('Learning tree models from genotype data...\n')

if nargin == 2
    VIM_gen = GENIE3_mark(expr_matrix,gen_matrix);
elseif nargin == 3
    VIM_gen = GENIE3_mark(expr_matrix,gen_matrix,input_idx);
elseif nargin == 4
    VIM_gen = GENIE3_mark(expr_matrix,gen_matrix,input_idx,tree_method);
elseif nargin == 5
    VIM_gen = GENIE3_mark(expr_matrix,gen_matrix,input_idx,tree_method,K);
else
    VIM_gen = GENIE3_mark(expr_matrix,gen_matrix,input_idx,tree_method,K,ntrees);
end

%% Product and sum of importances
